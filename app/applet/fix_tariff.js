                        ...
                        \` : \`
                            <div class="flex items-center gap-3 animate-fade-in mt-2 border-t border-primary-100 pt-3">
                                <div>
                                    ...
                                    <div class="flex items-center gap-2">
                                        <input type="number" min="0" max="100" class="w-20 px-3 py-2 rounded-xl border border-primary-200 focus:border-primary-400 focus:ring-2 focus:ring-primary-100 outline-none text-center font-bold" value="\${em.tariff_details ? em.tariff_details.percentage_value : 40}" onchange="updateMasterCommission(\${em.id}, this.value)">
...
code = code.replace(
    /<div class="mb-6 bg-primary-50 p-5 rounded-2xl border border-primary-100">[\s\S]*?<p class="text-xs text-system-muted ml-2">Применяется ко всем услугам по умолчанию<\/p>\s*<\/div>\s*<\/div>/,
    newMasterUI
);

// We need to add the window.updateMasterTariff functions
const newFunctions = \`
...
// Fix updateMasterCommission behavior to also modify m.tariff_details.percentage_value
code = code.replace(
    'm.commissionRate = rate;',
    'm.commissionRate = rate; if(!m.tariff_details) m.tariff_details = {}; m.tariff_details.percentage_value = rate;'
);

// We also need to fix `lookupMasterServiceCommission` to use `m.tariff_details?.percentage_value`
code = code.replace(
...
console.log('done master tariff UI');
